...
def sampler(gen_seed,inf_seed,rows,cols,alpha,beta):
    dataset = dm.gen_dataset(gen_seed,rows,cols,alpha,beta)
    sample = gen_sample(inf_seed, dataset, num_iters,None,None)
    return sample
...
# block 5
def raw_testjob(gen_seed,inf_seed,rows,cols,alpha,beta):
    time.sleep(numpy.random.uniform(1))
    return sampler(gen_seed,inf_seed,rows,cols,alpha,beta)

# make memoized job (re-eval if the job code changes, or to reset cache)
testjob = Cloudless.memo.AsyncMemoize("testjob", ["gen_seed","inf_seed","rows","cols","alpha","beta"], raw_testjob, override=True)
...
for (k, v) in testjob.iter():
    xs.append(k[0])
    deltaT = sum([x["timing"]["zs"]["stop"]-x["timing"]["zs"]["start"] for x in v])
    ys.append(deltaT)
