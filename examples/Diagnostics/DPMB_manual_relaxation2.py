...
infer_state = ds.DPMB_State(gen_seed=0,
                            num_cols=NUM_COLS,
                            num_rows=NUM_ROWS,
                            init_alpha=hf.mle_alpha(4,2), ## 1.0,
                            init_betas=np.repeat(0.1,NUM_COLS),
                            init_z=1,
                            init_x=gen_state.getXValues(),
                            alpha_min=1)

kernel = dm.DPMB(inf_seed=0, state=infer_state, infer_alpha=False, infer_beta=False)
    
#inspect p_z_0

    
plot_helper("infer_mle_no_alpha_beta", infer_state)
kernel.transition()
